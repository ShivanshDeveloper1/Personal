...
function AboutSection() {
  ...
  const { scrollYProgress } = useScroll({
    target: ref,
    // Adjusted offset so it finishes animating before scrolling out of view
    offset: ["start 85%", "end 50%"], 
  });

  const railHeight = useTransform(scrollYProgress, [0, 1], ["0%", "100%"]);

  const text = `I’m a passionate Web Developer who loves turning ideas into modern, responsive, and user-friendly digital experiences. I enjoy solving problems through code, learning new technologies, and building websites that are both visually appealing and technically powerful.`;

  const words = text.trim().split(/\s+/);

  return (
    <div ref={ref} className="relative max-w-5xl mx-auto px-6 md:pl-12">
      {/* Scroll-progress rail */}
      <div className="absolute left-0 top-2 bottom-2 w-1 bg-gray-100 dark:bg-gray-800 hidden md:block rounded-full overflow-hidden">
        <motion.div
          style={{ height: railHeight }}
          className="w-full bg-gradient-to-b from-blue-600 via-purple-600 to-indigo-600"
        />
      </div>

      <h2 className="text-sm font-bold uppercase tracking-widest text-green-500 mb-6 md:pl-8 flex items-center gap-2">
        <span className="w-2 h-2 bg-green-500 rounded-full md:hidden"></span>
        About Me
      </h2>

      {/* Changed to flex-wrap for perfect spacing, adjusted line-height (leading-[1.2]) */}
      <p className="flex flex-wrap gap-x-2 md:gap-x-3 gap-y-1 text-2xl md:text-4xl font-semibold leading-[1.3] md:leading-[1.2] tracking-tight text-gray-900 dark:text-white md:pl-8">
        {words.map((word, i) => (
...
function Word({ word, progress, index, total }) {
  ...
  const y = useTransform(progress, [start, end], [12, 0]); // Increased Y distance slightly for more impact
  const filter = useTransform(blurAmount, (v) => `blur(${v}px)`);

  return (
    <motion.span
      style={{ opacity, filter, y }}
      className="will-change-[filter,opacity,transform] transition-opacity duration-75"
    >
      {word}
    </motion.span>
  );
  ...
  return (
    <>
      ...
      <div className="container max-w-7xl mx-auto py-12 px-4 overflow-hidden">
        {/* 1. HERO SECTION */}
        <HeroDiv className="text-center pt-10">
        ...
        {/* 2. ABOUT ME SECTION (Spacing vastly improved here) */}
        <div className="my-24 md:my-32">
          <AboutSection />
        </div>

        {/* 3. STATS BAR */}
        <StaggerGrid className="grid grid-cols-2 md:grid-cols-3 gap-4 max-w-4xl mx-auto mb-24">
        ...
        {/* 4. BIO SECTION */}
        <SectionFadeInUp className="mb-24">
        ...
        {/* ... Rest of your sections (Expertise, Experience, Education) remain unchanged ... */}
        
        {/* 5. EXPERTISE CARDS */}
        <SectionFadeIn className="mb-24" delay={0.2}>
        ...
        {/* 6. EXPERIENCE */}
        <SectionFadeIn className="mb-24" delay={0.4}>
          <HeadingFadeInUp className="text-3xl font-bold text-center mb-10">
        ...
        {/* 7. EDUCATION */}
        <SectionFadeIn delay={0.6} className="pb-12">
